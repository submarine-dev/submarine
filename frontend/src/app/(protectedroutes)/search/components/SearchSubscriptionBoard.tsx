...
export const SearchSubscriptionBoard: FC<Props> = ({ listOfSubscriptions }) => {
  const [searchText, setSearchText] = useState<string>("");
  const [searchResults, setSearchResults] = useState<SubscriptionBaseType[]>([]);
  ...
  const searchHandler = () => {
    ...
    const newSearchResults = listOfSubscriptions.filter((data) =>
      data.subscriptionName.toLowerCase().includes(searchTextLowerCase),
    );
  ...
  return (
    <BoardWrapper>
      ...
      {searchResults.length === 0 && listOfSubscriptions.length === 0 && searchText !== "" ? (
        <p className='m-3'>該当するサブスクリプションは見つかりませんでした</p>
      ) : (
            ...
            {searchResults.map((subscriptionItem, index) => {
              return (
                <Drawer key={subscriptionItem.subscriptionId}>
                  ...
                  <DrawerContent>
                    <p className='mx-5 pb-2'>新しいサブスクリプションを追加</p>
                    <div className='border-b flex gap-2 mx-5 pb-2'>
                      <img
                        ...
                        height={24}
                      />
                      <p>{subscriptionItem.subscriptionName}</p>
                    </div>
                    <div className='flex flex-col items-center gap-4 p-5'>
                      ...
                      <DrawerClose className='w-full flex justify-center gap-4'>
                        ...
                        <Button
                          disabled={selectedSubscriptionPlanId === null}
                          onClick={() => {}}
                          className='w-[30%]'
                        >
                          追加する
